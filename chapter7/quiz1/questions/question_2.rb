...
#   for the number of a group to print out.
  ...
  def instruct_user_to_enter_num_of_groups
  ...
  def instruct_user_to_enter_a_name
    puts "Enter a name"
  end

  def instruct_user_to_enter_group_to_print
    puts "Enter the number of a group to print out"
  end

  def run_program
    instruct_user_to_enter_num_of_groups
    acquire_user_input
    num = $input.to_i
    ...
    loop do
      instruct_user_to_enter_a_name
      acquire_user_input
      $groups_array[index % num] << $input if $input != "stop"
      index += 1
      break if $input == "stop"
    end
    ...
    $input = ""
    while $input != "stop"
      instruct_user_to_enter_group_to_print
      acquire_user_input
      if $input == "stop"
        break
      end
      $input = $input.to_i
      puts $groups_array[$input - 1].join(", ")
    end
  end
